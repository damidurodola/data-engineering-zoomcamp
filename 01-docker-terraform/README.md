...
Docker is a containerization platform that bundles an application and all of its dependencies into an isolated unit called a container.
...
```
**Step 2:** Use a Virtual Envirnment
uv is a modern Python package manager that is faster than pip.
```
pip install uv
```
To add dependencies
...
**Step 3:** Running PostgreSQL with Docker .
...
**Step 1:** Exploration of our dataset, to check if our results matches the ones presented in the lesson.
```sql
...
**Step 3:** ingest the data using the Python script.
```
...
**Step 1:** [install docker compose](https://docs.docker.com/compose/install/linux/#install-using-the-repository). Docker-compose helps to launch multiple containers with a single configuration file.
